...
% axis("equal")
% title("A FEM solution")
% clim([0, 2])
% 
% figure
% 
% contour(xx, yy, u_init, "LineWidth", 1)
% axis("equal")
...
% xlim([0, 1])
% ylim([0, 1])
% title("A PINN solution", "FontSize", 14)
...
% saveas(gcf, "./u1.png")
ax = gca;
exportgraphics(ax,'u1_new.png','Resolution',300) 




function out = f(location, state)
out = -state.u .^ 2 + 1600 * sin(pi*location.x) .* sin(pi*location.y);
end
