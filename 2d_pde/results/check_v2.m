...
model.SolverOptions.ResidualTolerance = 2e-4;
...
xlim([0, 1])
ylim([0, 1])
title("A FEM solution", "FontSize", 14)
xlabel("$x$", "FontSize", 14, "Interpreter", "latex")
ylabel("$y$", "FontSize", 14, "Interpreter", "latex")
...
out = -state.u .^ 2 + 4 * sin(pi*location.x) .* sin(pi*location.y);
end
...
data = load("params_case12_1k_r2.mat");
theta = data.theta;
idx = 262; % 24, 31, 47, 79
