...
%% Case A:
% nn = @(location) pinn(location, 185);
% setInitialConditions(model, nn);
% 
% model.SolverOptions.ResidualTolerance = 2e-13;
...
y = linspace(0, 1, 200);
[xx, yy] = meshgrid(x, y);
u = interpolateSolution(results2, xx, yy);
u = reshape(u, [200, 200]);
disp([idx, max(max(abs(results2.NodalSolution)))])
% disp([idx, max(max(u))])
...
% saveas(gcf, "./u1.png")
ax = gca;
% exportgraphics(ax,'u1.png','Resolution',300) 
