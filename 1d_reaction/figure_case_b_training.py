...
model = torch.load("./checkpoints_b/model_0_case3")
u_pred = model.forward(xx).detach().cpu().numpy()
us = [u_pred[:, 20, :]]
for i in range(10000):
    if (i+1) % 50 == 0:
        model = torch.load("./checkpoints_b_training/model_{}".format(str(i+1)))
        u_pred = model.forward(xx).detach().cpu().numpy()
        us += [u_pred[:, 20, :]]
...
# ax.set_ylabel("$u$")
plt.savefig("./results/case_b_training.png", bbox_inches="tight")
