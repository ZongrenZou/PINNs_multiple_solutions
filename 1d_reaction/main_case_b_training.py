...
np.random.seed(76229)
torch.manual_seed(22231)
...
u_test = data["u_test"]
f_test = data["f_test"]
D = float(data["D"].reshape([]))
kappa = float(data["kappa"].reshape([]))
w = float(data["w"].reshape([]))
x_f_train = x_test
f_train = f_test
x_u_train = np.array([-1, 1]).reshape([-1, 1])
u_train = np.array([u_test[0, 0], u_test[-1, 0]]).reshape([-1, 1])
print(u_test.shape, D, w)
...
torch.save(model, "./checkpoints_b/model_0_case3_training")
...
def update():
    ...
    def loss_function():
        ...
        loss_f = torch.mean((D * u_xx + kappa * torch.tanh(u) - f_train) ** 2)
...
def train(): 
    ...
    for i in range(niter):
        ...
        if (i + 1) % 50 == 0:
            ...
            torch.save(model, "./checkpoints_b_training/model_{}".format(str(i+1)))
            model.train()
...
torch.save(model, "./checkpoints_b/model_1_case3_training")
