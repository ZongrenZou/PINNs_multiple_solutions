...
torch.save(model, "./checkpoints/model_0_case2")
...
def update():
    ...
    def loss_function():
        ...
        res = -eps * (u_xx + u_yy) + u ** 3 - u
        # loss_f = torch.mean(res ** 2)
        loss_f = torch.mean(res ** 2) + torch.mean((u_b[:,:198, :] + 1) ** 2) + torch.mean((u_b[:,198:, :] - 1) ** 2)
...
def train(): 
    ...
    for i in range(niter):
        ...
        if (i + 1) % 1000 == 0:
            ...
            torch.save(model, "./checkpoints/model_1_case2")
