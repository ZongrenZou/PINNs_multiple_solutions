...
np.random.seed(77663)
torch.manual_seed(22331)
...
# R = 3
# model = models.PNN2(units=50, n=n, R=R).to(device)
torch.save(model, "./checkpoints_training/model_0")
...
def train(): 
    ...
    for i in range(niter):
        ...
        if (i + 1) % 10 == 0:
            ...
            torch.save(model, "./checkpoints_training/model_{}".format(str(i+1)))
            model.train()
...
torch.save(model, "./checkpoints_training/model_end")
u_pred = model.forward(
    torch.tensor(x_test, dtype=torch.float32, device=device),
).detach().cpu().numpy()
